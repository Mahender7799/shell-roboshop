...
SG_ID=sg-08bd67a1891eddf3d


for instance in $@
do
  INSTANCE_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t3.micro --security-group-ids 
  $SG_ID --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$instance}]" --query 
  'Instances[0]' --output text)
  
  if [ $instance != "frontend" ];then
       IP=$(aws ec2 description-instances --instance-ids $INSTANCE_ID --query 
